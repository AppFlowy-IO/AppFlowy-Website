...
function DevInfoSection() {
    ...
    return (
        <section className='dev-info-section w-full bg-night-blue py-16 sm:py-20 lg:py-24 xl:py-[120px]  px-[80px] max-xl:px-[4vw]'>
                    ...
                    >
                        {tabOptions.map((tab) => (
                            <MuiTab
                                ...
                                key={tab.value}
                                value={tab.value}
                                sx={{ width: '100%', maxWidth: '540px', minWidth: 0, padding: 0, textTransform: 'none', alignItems: 'stretch' }}
                                label={
                                    <div
                                        className={`flex w-full items-center justify-between gap-4 rounded-2xl border px-6 py-7 text-left transition-colors ${value === tab.value ? 'border-[#BEBEBE] bg-[#0F0A1E]' : 'border-[#2D2B3B] bg-[#161325] hover:bg-gray-03'
                                            }`}
                                    >
                                        <div className='flex items-center gap-4'>
                                            <div
                                                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl transition-colors ${value === tab.value ? 'bg-white text-night-blue' : 'bg-gray-10 text-white'
                                                    }`}
                                            >
                                                {tab.icon}
                                            </div>
                                            <span className='whitespace-nowrap font-mono text-style-h5 font-semibold text-white'>{tab.label}</span>
                                        </div>
                                        {value === tab.value && <span className='shrink-0 text-white'><TabArrowIcon /></span>}
                                    </div>
                                }
                            />
                        ))}
                            ...
                            <TabPanel key={panel.value} index={panel.value} value={value}>
                                <div className='flex w-full flex-col '>
                                    <div className='relative h-[300px] w-full shrink-0 overflow-hidden rounded-2xl max-md:h-[200px]'>
                                        <Image loading={'eager'} fill sizes='(max-width: 1024px) 100vw, 50vw' className='object-cover' src={panel.image.src} alt={panel.title} />
                                    </div>
                                    <div className='mt-8 flex flex-col gap-3'>
                                        <h2 className='text-style-h2 font-semibold text-white'>{panel.title}</h2>
                                        <p className='whitespace-pre-wrap text-style-h5 text-gray-40'>{panel.desc}</p>
                                    </div>
                                </div>
                            </TabPanel>
                        ))}
