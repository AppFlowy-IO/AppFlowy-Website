        ...
        <div className={`flex flex-col gap-4 ${isLabelAtEnd ? 'items-end' : 'items-start'}`}>
            ...
            >
                <Image
                    ...
                    width={52}
                    height={52}
                    className={`h-13 w-13 object-contain ${highlighted ? '' : 'opacity-75 grayscale'}`}
                />
            </span>
            <span
                className={`text-center text-2xl uppercase font-bold ${highlighted ? 'text-primary' : 'text-[#9CA0AA]'} max-md:text-lg`}
            >
                {name}
            </span>
        </div>
